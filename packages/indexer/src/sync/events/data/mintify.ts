...
import { Mintify } from "@reservoir0x/sdk";
...
export const orderCancelled: EventData = {
  kind: "seaport",
  subKind: "mintify-order-cancelled",
  addresses: { [Mintify.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const ordersMatched: EventData = {
  kind: "seaport",
  subKind: "mintify-orders-matched",
  addresses: { [Mintify.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const counterIncremented: EventData = {
  kind: "seaport",
  subKind: "mintify-counter-incremented",
  addresses: { [Mintify.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
...
export const orderValidated: EventData = {
  kind: "seaport",
  subKind: "mintify-order-validated",
  addresses: { [Mintify.Addresses.Exchange[config.chainId]?.toLowerCase()]: true },
